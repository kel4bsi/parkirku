...
import parkir.service.AdminDao; 
...
statement.setString(2, admin.getPassword()); resultSet = 
statement.executeQuery(); connection.commit(); 
...
} else { throw new AdminException("Data Admin Tidak Ada!!!"); 
} connection.commit(); 
return admin;
} catch(SQLException exception) { 
...
} catch (SQLException ex) { } if (statement != null) 
{ try { statement.close(); 
